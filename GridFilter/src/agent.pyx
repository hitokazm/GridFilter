...
class Agent(object):
    ...
    def __init__(self, bzrc):
        ...
        self.grid_size = 16
        self.grid_step = 0.5
        self.upper_theshold = 0.99999999
        self.lower_theshold = 0.00000001
        self.speed = 1
...
#                self.grids.append(grid)
    ...
    def update_probabilities(self, tank):
        ...
        cdef int center_x, center_y, sensor_x, sensor_y, row, rows, col, cols
        cdef DTYPE_t true_positive, true_negative, occupied, unoccupied
        ...
        for row in xrange(rows):
            for col in xrange(cols):
                ...
                if sensor_x < 0 or sensor_y < 0 or sensor_x > 799 or sensor_y > 799:
                    ...
                    if self.priors[sensor_x][sensor_y] < self.upper_theshold and self.priors[sensor_x][sensor_y] > self.lower_theshold:
    ...
    def set_probablity(self, p, x, y):
        ...
        #return p
        return self.check_surrounding_cells(p, x, y)
    ...
    def get_distance(self, tank):
        ...
        #print self.prev
        cdef double d = math.sqrt((tank.x - self.prev[tank.index][0])**2 + (tank.y - self.prev[tank.index][1])**2)
        #d = (tank.x - self.prev[tank.index][0])**2 + (tank.y - self.prev[tank.index][1])**2
        return d
    
    def is_stuck(self, tank):
        """ check if the tank is moving """
        cdef double d = self.get_distance(tank)
        if self.debug == True:
            print "Distance: %.2f" % d
            ...
            for j in xrange(len(self.grids[k])): # rows in self.grids
                for i in xrange(len(self.grids[k][j])): # cols in self.grids
                    ...
                    if self.flags[k][j][i].poss_color != self.constants['team']:
                        ...
                        if d < self.distance_factor * ((self.occgrid_width/2) ** 2):
                            ...
                            #self.flags[k][j].remove(self.flags[k][j][i])
